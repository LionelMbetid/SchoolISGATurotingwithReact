...
export default function ListTutors() {
  ...
  useEffect(() => {
    ...
    axiosClient.get(`tutor/getAllTutors/${user.id}`,{ params: { page: page } }).then(({ data }) => {
      setIsLoading(false);
  ...
  function search(e) {
    ...
    console.log(keyword);
    setIsLoading(true);
    axiosClient.get(`tutor/getAllTutors/${user.id}/${keyword}`,{ params: { page: page } }).then(({ data }) => {
      setIsLoading(false);
      setTutorsList(data.data.data);
      setPage(data.data.current_page);
    }).catch(err => {
  ...
  return (
    <div class="p-4">
      <div className="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-black dark:text-white">Liste des Tuteurs</h1>
